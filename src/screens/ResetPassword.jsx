...
const ResetPassword = () => {
  ...
  const handleSubmit = async () => {
    if (!password || !rePassword) {
      Toast.show({ type: 'error', text1: 'Error', text2: 'Please enter both passwords.' });
      return;
    }
    if (password !== rePassword) {
    ...
    try {
      const response = await postData('/api/user/set-password', { password });
      
      if(response?.Status || response?.success){
        Toast.show({ type: 'success', text1: 'Success', text2: 'Password updated successfully!' });
        navigation.goBack();
      } else {
        Toast.show({ type: 'error', text1: 'Error', text2: response?.message || 'Failed to update password.' });
      }
    } catch (error) {
      console.error(error);
